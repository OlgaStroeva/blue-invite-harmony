...
const ParticipantForm = () => {
  ...
  useEffect(() => {
    ...
    if (eventId) {
      ...
      fetch(`http://158.160.171.159:7291/api/events/${eventId}`, {
      ...
      fetch(`http://158.160.171.159:7291/api/forms/get-by-event/${eventId}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
          .then(res => res.ok ? res.json() : null)
          .then(data => {
  ...
  const handleSubmit = async (data: Record<string, string>) => {
    ...
    try {
      ...
      const response = await fetch(`http://158.160.171.159:7291/api/forms/add-participant/${formId}`, {
        method: "POST",
        headers: {
