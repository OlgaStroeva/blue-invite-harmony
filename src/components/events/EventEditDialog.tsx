...
const EventEditDialog = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(import.meta.env.VITE_API_URL +`/api/events/update/${event.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_API_URL +`/api/events/delete/${event.id}`, {
        method: "DELETE",
        headers: {
