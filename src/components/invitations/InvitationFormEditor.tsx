...
const InvitationFormEditor = ({
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      const formRes = await fetch(`https://0.0.0.0:7291/api/forms/get-by-event/${eventId}`, {
        headers: {
      ...
      if (formRes.ok) {
        ...
        const createRes = await fetch(`https://0.0.0.0:7291/api/forms/create/${eventId}`, {
          method: "POST",
          headers: {
      ...
      try {
        const response = await fetch(`https://0.0.0.0:7291/api/forms/update-form/${formId}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
