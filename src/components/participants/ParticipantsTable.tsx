...
const ParticipantsTable = ({ open, onOpenChange, event }: ParticipantsTableProps) => {
  ...
  const handleSendInvite = async (participantId: number) => {
    ...
    try {
      ...
      const inviteRes = await fetch(import.meta.env.VITE_API_URL +`/api/invitations/send/${formId}/${participantId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleCancelInvite = async (participantId: number) => {
    ...
    try {
      ...
      const cancelRes = await fetch(import.meta.env.VITE_API_URL +`/api/invitations/cancel/${formId}/${participantId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  useEffect(() => {
    ...
    fetch(import.meta.env.VITE_API_URL +`/api/forms/participants/${event.id}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const handleXlsxUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(import.meta.env.VITE_API_URL +`/api/forms/upload-participants/${event.id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`
        },
  ...
  const updateParticipantOnServer = async (participantId: number, updatedData: Record<string, string>) => {
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(import.meta.env.VITE_API_URL +`/api/forms/update/${participantId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
