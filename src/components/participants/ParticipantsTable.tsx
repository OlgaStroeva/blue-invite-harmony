...
const ParticipantsTable = ({ open, onOpenChange, event }: ParticipantsTableProps) => {
  ...
  const handleSendInvite = async (participantId: number) => {
    ...
    try {
      ...
      const formRes = await fetch(`https://158.160.171.159:7291/api/forms/get-by-event/${event.id}`, {
        headers: {
      ...
      const inviteRes = await fetch(`https://158.160.171.159:7291/api/invitations/send/${formId}/${participantId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleCancelInvite = async (participantId: number) => {
    ...
    try {
      ...
      const cancelRes = await fetch(`https://158.160.171.159:7291/api/invitations/cancel/${formId}/${participantId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  useEffect(() => {
    ...
    fetch(`https://158.160.171.159:7291/api/forms/participants/${event.id}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const handleXlsxUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(`https://158.160.171.159:7291/api/forms/upload-participants/${event.id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`
        },
  ...
  const updateParticipantOnServer = async (participantId: number, updatedData: Record<string, string>) => {
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://158.160.171.159:7291/api/forms/update/${participantId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
